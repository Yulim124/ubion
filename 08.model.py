...
def show_sidebar():
    ...
    age = st.sidebar.slider('나이(age)', 0, 99, 29)  # 0 ~ 99 / base 처음 시작값 29
...
def showDiabetes():
    ...
    st.markdown("<h4 style='color:white;'>당뇨병 데이터(참고):머신러닝 학습데이터</h4>", unsafe_allow_html=True)
...
def ml_Diabetes(features):
    ...
    # 예측
    prediction = load_clf.predict(features)   # 1 : 당뇨, 0 : 당뇨아님
